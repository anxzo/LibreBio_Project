...
|------|------|----------|----------|----------|------|----------|
| Phase 0 | 项目基础设施 | ✅ 100% | ✅ 100% | ✅ 100% | ✅ 100% | 100% |
| Phase 1 | 核心数据类型 | ✅ 100% | ✅ 100% | ✅ 100% | ✅ 100% | 100% |
| Phase 2 | 基本格式 I/O | ❌ | ❌ | ❌ | ❌ | 0% |
...
- 代码审查与文档检查（全 9 模块）：M01–M06 + M16–M18 全部通过
- 审查结论：无严重问题，无待修复项；文档 Doxygen 注释全部齐备
- Phase 0 模块审查覆盖率 3/3 (100%)，Phase 1 模块审查覆盖率 6/6 (100%)
...
### 2026-05-15 (更新) — v0.4.0

- 完成 M16 CLI::App 实现：19 项测试用例全部通过
- 新增头文件 include/libre_bio/cli/app.h，实现文件 src/cli/app.cpp，测试文件 test/cli/app_test.cpp
- CMakeLists.txt 注册 app.cpp 到 libre_bio_core 库 + cli_app_test 测试目标 + lint/analyze 静态分析
- API 采用游标模式：add_option 自动关联到最后注册的子命令，未注册子命令时防御性忽略
- 提供 parse(std::vector<std::string>) 测试重载，方便测试代码传递字符串列表
- 支持组合短选项（-abc）、长选项 = 语法（--input=file.txt）、短选项值贴附（-ofile.txt）
- 支持 -- 终止符：后续参数强制作为位置参数
- 重复选项后者覆盖，未提供选项回退到默认值
- PIMPL 模式隔离实现细节，移动语义正确转移所有权
- 更新详细设计文档 m16_cli_app.md → v0.3.0（游标模式 + 防御机制 + 测试重载）
- clang-tidy 零新增警告，cppcheck 零警告
- 完整测试通过率：108/108 (100%)
